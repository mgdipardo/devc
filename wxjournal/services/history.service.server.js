module.exports = function(app) {
    ...
    app.get("/api/station/:stationId/history", findHistoriesByStationId);

    function findHistoriesByStationId(req, res) {
        var stationId = req.params.stationId;
        var stationHistories = [];
        for(var h in histories) {
            if(histories[h].stationId === stationId) {
                stationHistories.push(histories[h]);
            }
        }
        res.json(stationHistories);
    }
};
